...
# Load function
source('code/sortAffiliations-WoS.R')

# Set variables needed to run function
table=sortAffiliations(file="Marine-WoS.bib", n = 1000, GNout="Marine-WoS_GN.bib", GSout="Marine-WoS_GS.bib", dir="bibs")
...
# Plot affiliations for all 890 citations
(all890=ggplot(table, aes(x=country))+
  geom_bar()+
...
top=ggplot(top25, aes(x=country))+
  geom_bar()+
  facet_grid(.~designation, scales = "free_x", space = "free")+
  theme_bw()+
  theme(axis.text.x = element_text(angle=90, vjust=0.5),
